...
#detecting the outliers 'outlierDetect' ##===========  #df[which(df$code=="E01009014"),]
...
prop_crime_per000_people <- props(crime_per_000_people, id_field = TRUE, scale = 1)  #head(prop_crime_per000_people) #prop_crime_per000_people[,2]
...
#------------------------------------------------------------------------------------------------------------------
#Import Birmingham study area
data <- read.table("C:/Users/monsu/Documents/GitHub/Packages/inter_comparison_Micro/data1_1/wm_bm_PC_OA_.csv", sep=",", head=TRUE)  
head(data) #nrow(data)
data <- cbind(data$code, data[,4:ncol(data)]) #head(data)
head(data)
colnames(data) <- c("code", paste("p", 2002:2016, sep=""))
head(data)

#read the attribute table of the ESRI data (for 'study_area1'), including the population data
df <- read.table("C:/Users/monsu/Documents/GitHub/Packages/inter_comparison_Micro/data1_1/population_WM_OA_02_16.csv", sep=",", head=TRUE)#head(df) #nrow(df)
...
head(data)
data <- cbind(data$code, data[, 18:32])
colnames(data) <- c("code", paste("p", 2002:2016, sep=""))
head(data)
...
df <- read.dbf(paste("C:/Users/monsu/Documents/GitHub/Packages/v013/data2/shp2/",
                     study_Area, "LSOA_", "pop_02_12.dbf", sep="")) #nrow(df)#head(df)
uni_codes <- as.vector(df$code)
...
all_ <- NULL
for(t in 1:length(uni_codes)){ #t<-1
  id_ <- which(others$SCOTLAND==uni_codes[t])
  sub_ <- c(as.numeric(as.character(df[t,1:(ncol(df)-1)])), 
            as.numeric(as.character(others[id_,2:ncol(others)])))
  all_ <- rbind(all_, sub_)
}
row.names(all_) <- uni_codes
colnames(all_) <- paste("p", 2002:2016, sep="") #head(all_)#nrow(all_)
#--------------------------------------
head(df)
#move the first row
df <- as.data.frame(cbind(df$code, df[,1:(ncol(df)-1)]))   #df[which(df[,1]=="E01009510"),]
...
setwd("C:/Users/monsu/Documents/GitHub/Packages/inter_comparison_Micro/")
...
tsd_ind_ <- function(code = code, unit = "LSOA", n_classes = 10){

#import the 2011 data
if(unit == "LSOA"|unit == "DZ"){
ukdata_ <- read.csv(file="Dataset- 2011 UK LSOA.csv", sep=",", head=TRUE) #head(ukdata_)  #nrow(ukdata_)
}
