...
sum(as.numeric(as.character(crime_per_200_people[,2])))
...
colSums(prop_crime_per200_people[,2:ncol(head(prop_crime_per200_people))])
```
...
In the following sections, we provide a work example of clustering with `akmedoids.clust` function using the `prop_crime_per200_people` data plotted in Figure \ref{fig:figs4}. The `statPrint` function will then be used generate the descriptive summary of the clusters and `spatialPlt` function to generate the spatial manifestation of each group. 

```{r figs4, echo=TRUE, fig.width=8,fig.height=4,fig.align="center", fig.cap="\\label{fig:figs4} Trajectory of crime proportions per location"}
...
head(prop_crime_per200_people_melt)#previewing the first few rows
...
***Data***: 
Each trajectory in Figure \ref{fig:figs4}of row in the `"prop_crime_per200_people"` represents the proportion of data describes derived above 

It should be noted, the interpretation will vary. Note:
