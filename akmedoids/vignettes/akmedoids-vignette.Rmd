...
abstract: The `akmedoids` advances a set of R-functions for longitudinal clustering
  ...
  In this document, we provide a guide towards using this package for measuring long-term inequalities in the exposure to crime at small geographical area levels. 
vignette: |
...
Given the vector of group membership (labels) and its corresponding trajectory object data matrix indexed correspondingly, the `statPrint` function reports the descriptive as well as change statistics of each group. The function can also generates the plots of the group membership and performances over time.

Using the output from the `akmedoids.clust`, the `spatialPlt` function can be executed as follows:


```{r figs7, eval=TRUE, echo=FALSE, include=FALSE, fig.width=8,fig.height=4,fig.align="center", fig.cap="\\label{fig:figs7}, group memberships"}
...
```{r figs8, echo=TRUE, fig.width=8,fig.height=4,fig.align="center", fig.cap="\\label{fig:figs8}, group performance"}

print(statPrint(clustr, prop_crime_per200_people, id_field=TRUE, 
                bandw = 0.60, type="stacked"))
