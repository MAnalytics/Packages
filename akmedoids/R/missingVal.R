...
#' @usage missingVal(traj, id_field = FALSE, method = 2, replace_with = 1, fill_zeros = FALSE)
#' @details Given a matrix or data.frame with some missing values represented by \code{(NA, Inf, 0)}, the function \code{missingVal} determines the missing values using either the \code{arithmetic} or \code{regression} method.
#' @examples
#' traj <- gm.crime.sample1
#' print(traj)
#' missingVal(traj, id_field = TRUE, method = 2, replace_with = 1, fill_zeros = FALSE)
#' @rawNamespace import(stats)
#' @return A data.frame with missing values \code{(NA, Inf, 0)} filled up
#' @export


missingVal <- function(traj, id_field = FALSE, method = 2, replace_with = 1, fill_zeros = FALSE){  #id_field = TRUE; replace_with = "Mean_row"
