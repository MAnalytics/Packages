...
#' @usage missingValue(traj, id_field = FALSE, method = 2, replace_with = 1, fill_zeros = FALSE)
...
#' print(traj)
#' missingValue(traj, id_field = TRUE, method = 2, replace_with = 1, fill_zeros = FALSE)
#' @rawNamespace import(stats)
#' @return A data.frame with missing values \code{(NA, Inf, 0)} filled up
#' @export


missingValue <- function(traj, id_field = FALSE, method = 2, replace_with = 1, fill_zeros = FALSE){  #id_field = TRUE; replace_with = "Mean_row"
