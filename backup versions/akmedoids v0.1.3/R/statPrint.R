...
#' @param show.plot [TRUE or FALSE] Whether to show plots
#' @examples
#' print(traj)
...
#' @references \code{2}. Wickham H. (2016). Elegant graphics for Data Analysis. Spring-Verlag New York (2016).
#' @keywords plot, clusters
#' @rawNamespace import(reshape2, ggplot2, stats)
#' @export

statPrint <- function(clustr, traj, id_field=TRUE, reference = 1, N.quant = 4, show.plot=TRUE, type = "lines", y.scaling="fixed"){
...
gr_medoid <- NULL
  ...
  quant1=rank_negative[intervals_fall]
  quant2=rank_positive[intervals_rise]
  quant = c(quant1, quant2)
  quant <- data.frame(quant)
  colnames(quant) <- c(paste("Qtl:","1st-",N.quant, "th", sep=""))
  if(N.quant==2){
  colnames(quant) <- c(paste("Qtl:","1st-",N.quant, "nd", sep=""))
  }
  if(N.quant==3){
  colnames(quant) <- c(paste("Qtl:","1st-",N.quant, "rd", sep=""))
  }

  change_Stats <- cbind(change_Stats, quant)

  #------------------------------------------
  #Loop through "change_Stats" table and categorise the groups into three classes: 'Decreasing', 'Stable' & 'Increasing'
  class <- matrix(0, nrow(change_Stats),1)
    decr_grp <- which(change_Stats[,3]==0 & change_Stats[,4]!=0)
    stab_grp <- which(change_Stats[,3]!=0 & change_Stats[,4]!=0)
    incr_grp <- which(change_Stats[,3]!=0 & change_Stats[,4]==0)
    class[decr_grp] <- "Decreasing"
    class[stab_grp] <- "Stable"
  class[incr_grp] <- "Increasing"
  colnames(class) <- "Class"
  change_Stats <- cbind(change_Stats, data.frame(class))
  #------------------------------------------
