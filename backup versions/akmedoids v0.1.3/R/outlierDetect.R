...
outlierDetect <- function(traj, id_field = FALSE, method = 1, threshold = 0.95, count = 1, replace_with = 1){
  ...
  if(method==1){
    ...
    thres_ <- as.vector(round(quantile(as.numeric(as.character(as.vector(unlist(as.data.frame(dat))))), threshold), digits=10))
    id_ <- which(as.numeric(as.matrix(dat)) > thres_)
  ...
  # method: "manual"
  #------------------------------------
  if(method==2){  #threshold=180
    id_ <- which(as.numeric(as.matrix(dat)) > threshold)
    outlier_mat[id_] <- "TRUE"
  }
