...
#' @param multiplier [numeric] A quantify by which to the ratio \code{traj/denomin} is expressed. Default is \code{1}.
...
rates <- function(traj, denomin, id_field=TRUE, multiplier = 1){
  ...
  data1 <- apply(dat1[,2:ncol(dat1)], 2, as.numeric)#head(data1) #head(dat1)
