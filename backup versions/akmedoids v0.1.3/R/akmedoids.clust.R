...
#' @references \code{2}. Calinski T, Harabasz J (1974) A dendrite method for cluster analysis. Commun Stat 3:1-27.
#' @references \code{3}. Genolini, C.et al. (2016) Package ‘longitudinalData’
...
#If only one value of  k is provided, replicate it
  if(length(k)==1){
    k <- rep(k, 2)
  }
...
if(method=="linear"){
  ...
  if(k[1] < 3 | k[1] > nrow(traj) | k[2] > nrow(traj) |
    ...
    #-----------------------------------------------------
    #looping through to calculate the coefficients (slopes)
    sl_List <- NULL
...
if(method!="linear"){
  stop("Only 'linear' method option is currently available")
  }
